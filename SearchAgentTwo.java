...
public class SearchAgentTwo{
    ...
    public int pwaiter = 0;
    public int walkingnumber = 0;
	...
	public void getAroundWall(){
		...
		*/	
		...
		//should create a function that gets outa tunnels.	
		//if theres an opening one spot forward, and one spot to the left (diagonal)
		//move left one but make sure its an avaliable move, if not, then repeat 
		if (!wallAtHuh(11)){
			sendEffectorCommand("l");
			if(wallAtHuh(12)==true){
				getAroundWall();
			...
			if(wallAtHuh(12)==false){
				sendEffectorCommand("f");
			}
		}else{
			//if the left isnt open, check the right and do the same thing as above^^
			if (!wallAtHuh(13)){
				sendEffectorCommand("r");
				if(wallAtHuh(12)==true){
					getAroundWall();
				...
				//if neither front left, and front right is open, check forward one, and left TWO spaces.
				//and makes sure both moves are ok to do.
				if (!wallAtHuh(10)){
					sendEffectorCommand("l");
					if(wallAtHuh(12)==true){
						getAroundWall();
					...
					//If too squares to the left are not open, then just go to the right two squares
					// and make sure they are legal moves.
					sendEffectorCommand("r");
					if(wallAtHuh(12)){
						getAroundWall();
					}else{
	...
	public static void main(String args[]) throws Exception {
		SearchAgentTwo Rob = new SearchAgentTwo();
